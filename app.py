...
# Dropbox credentials
# NOTA: Usaremos tu token original pero con una configuración diferente para evitar el error de gRPC
...
# Desactivamos gRPC para evitar el error de content-type
...
# Función para crear el cliente de Dropbox con la configuración correcta
def get_dropbox_client():
    try:
        # Crear un cliente Dropbox con una configuración específica
        dbx = dropbox.Dropbox(
...
def get_or_create_dropbox_folder(folder_name):
    ...
    try:
        # Comprobar si la carpeta existe en Dropbox
        try:
...
def upload():
    ...
    if allowed_file(file.filename):
        ...
        if dbx:
            try:
                # Crear o verificar la carpeta en Dropbox
                folder_metadata = get_or_create_dropbox_folder("ArchivosSubidos")
                if folder_metadata:
                    # Leer el archivo y subirlo a Dropbox
